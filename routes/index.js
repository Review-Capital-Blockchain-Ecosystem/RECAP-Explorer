...
function route_get_tx_api(res, txid) {
  if (txid == settings.genesis_tx) {
    route_get_block(res, settings.genesis_block);
  } else {
    db.get_tx(txid, function(tx) {
...
function route_get_address_api(res, hash, count) {
  db.get_address(hash, function(address) {
    if (address) {
      var txs = [];
      var hashes = address.txs.reverse();
...
router.get('/ext/tx/:txid', function(req, res) {
  route_get_tx_api(res, req.param('txid'));
});
...
router.get('/getaddress/:hash', function(req, res) {
  route_get_address_api(res, req.param('hash'), settings.txcount);
});
